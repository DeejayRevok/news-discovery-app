...
class CBSSanFranciscoRSSNewsAdapter(SourceAdapter):
    __DATE_FORMAT: ClassVar[str] = "%a, %d %b %Y %H:%M:%S %z"
    __ROOT_NEW_TAG: ClassVar[str] = "item"

    def _fetch(self) -> Iterator[Element]:
        LOGGER.info("Fetching news from %s", self.source_params["cbs_sf_rss"])
        response = requests.get(self.source_params["cbs_sf_rss"])
    ...
    def _adapt_single(self, item: Element) -> New:
        ...
        content = self.__parse_content(raw_new_dict["ns2:encoded"])
